...
class Executor(AgentExecutor):
    def __init__(self):
        self.agent = Agent()
        logger.info("Calculator agent executor initialized")

    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        """Execute the calculator agent with the given context and event queue.
        
        The calculator agent processes mathematical operations using MCP tools.
        """
        try:
            ...
            logger.error(f"Error executing calculator agent: {str(e)}", exc_info=True)
