...
from magnet.knowledge.storage.base_knowledge_storage import BaseKnowledgeStorage
from magnet.rag.chromadb.config import ChromaDBConfig
from magnet.rag.chromadb.types import ChromaEmbeddingFunctionWrapper
from magnet.rag.config.utils import get_rag_client
...
from magnet.rag.embeddings.factory import build_embedder
from magnet.rag.embeddings.types import ProviderSpec
from magnet.rag.factory import create_client
from magnet.rag.types import BaseRecord, SearchResult
        ...
        try:
            ...
            if "dimension mismatch" in str(e).lower():
                Logger(verbose=True).log(
                    "error",
                    "Embedding dimension mismatch. This usually happens when mixing different embedding models. Try resetting the collection using `magnet reset-memories -a`",
                    "red",
                )
                raise ValueError(
                    "Embedding dimension mismatch. Make sure you're using the same embedding model "
                    "across all operations with this collection."
                    "Try resetting the collection using `magnet reset-memories -a`"
                ) from e
