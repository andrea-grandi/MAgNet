...
from magnet.agents.constants import FINAL_ANSWER_AND_PARSABLE_ACTION_ERROR_MESSAGE
from magnet.agents.parser import (
    AgentAction,
    AgentFinish,
    OutputParserError,
    parse,
...
from magnet.tools import BaseTool as NetAITool
from magnet.tools.base_tool import BaseTool
from magnet.tools.structured_tool import NetStructuredTool
from magnet.tools.tool_types import ToolResult
from magnet.utilities.errors import AgentRepositoryError
from magnet.utilities.exceptions.context_window_exceeding_exception import (
    LLMContextLengthExceededError,
)
from magnet.utilities.i18n import I18N
from magnet.utilities.printer import ColoredText, Printer
from magnet.utilities.types import LLMMessage
...
def parse_tools(tools: list[BaseTool]) -> list[NetStructuredTool]:
    ...
    Raises:
        ValueError: If a tool is not a NetStructuredTool or BaseTool.
    """
    tools_list: list[NetStructuredTool] = []

    for tool in tools:
        if isinstance(tool, NetAITool):
            tools_list.append(tool.to_structured_tool())
        else:
            raise ValueError("Tool is not a NetStructuredTool or BaseTool")

    return tools_list


def get_tool_names(tools: Sequence[NetStructuredTool | BaseTool]) -> str:
...
def render_text_description_and_args(
    tools: Sequence[NetStructuredTool | BaseTool],
) -> str:
...
def handle_context_length(
    ...
    if respect_context_window:
        ...
        printer.print(
            content="Context length exceeded. Consider using smaller text or RAG tools from magnet_tools.",
            color="red",
        )
        raise SystemExit(
            "Context length exceeded and user opted not to summarize. Consider using smaller text or RAG tools from magnet_tools."
        )
...
def _print_current_organization() -> None:
    ...
    if settings.org_uuid:
        ...
        console.print(
            "No organization currently set. We recommend setting one before using: `magnet org switch <org_id>` command.",
            style="yellow",
        )
...
def load_agent_from_repository(from_repository: str) -> dict[str, Any]:
    ...
    if from_repository:
        ...
        from magnet.cli.authentication.token import get_auth_token
        ...
        if response.status_code == 404:
            raise AgentRepositoryError(
                ...
                f"\nIf you are using the wrong organization, switch to the correct one using `magnet org switch <org_id>` command.",
            )
