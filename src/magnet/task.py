...
from magnet.events.event_types import (
    TaskCompletedEvent,
    TaskFailedEvent,
    TaskStartedEvent,
)
...
from magnet.utilities.config import process_config
from magnet.utilities.constants import NOT_SPECIFIED, _NotSpecified
from magnet.utilities.converter import Converter, convert_to_model
from magnet.utilities.guardrail import process_guardrail
from magnet.utilities.i18n import I18N
from magnet.utilities.printer import Printer
from magnet.utilities.string_utils import interpolate_only
...
class Task(BaseModel):
    ...
    Attributes:
        ...
        allow_magnet_trigger_context: Optional flag to control magnet_trigger_payload injection.
    ...
    allow_magnet_trigger_context: bool | None = Field(
        default=None,
        description="Whether this task should append 'Trigger Payload: {magnet_trigger_payload}' to the task description when magnet_trigger_payload exists in net inputs.",
    )
    ...
    def ensure_guardrail_is_callable(self) -> "Task":
        if callable(self.guardrail):
            ...
            from magnet.tasks.llm_guardrail import LLMGuardrail
    ...
    def handle_max_retries_deprecation(self):
        if self.max_retries is not None:
            warnings.warn(
                "The 'max_retries' parameter is deprecated and will be removed in NetAI v1.0.0. "
    ...
    def _execute_core(
        ...
        try:
            ...
            if not agent:
                raise Exception(
                    f"The task '{self.description}' has no agent assigned, therefore it can't be executed directly and should be executed in a Net using a specific process that support that, like hierarchical."
                )
            ...
            magnet_event_bus.emit(self, TaskStartedEvent(context=context, task=self))
            ...
            net = self.agent.net  # type: ignore[union-attr]
            if net and net.task_callback and net.task_callback != self.callback:
                net.task_callback(self.output)
            ...
            self.end_time = datetime.datetime.now()
            magnet_event_bus.emit(self, TaskFailedEvent(error=str(e), task=self))
    ...
    def prompt(self) -> str:
        ...
        should_inject = self.allow_magnet_trigger_context

        if should_inject and self.agent:
            net = getattr(self.agent, "net", None)
            if net and hasattr(net, "_inputs") and net._inputs:
                trigger_payload = net._inputs.get("magnet_trigger_payload")
...
- Use ```language for code blocks"""
    ...
    def interpolate_inputs_and_add_conversation_history(
        ...
        if inputs.get("net_chat_messages"):
            conversation_instruction = self.i18n.slice(
                "conversation_history_instruction"
            )

            net_chat_messages_json = str(inputs["net_chat_messages"])

            try:
                net_chat_messages = json.loads(net_chat_messages_json)
            except json.JSONDecodeError as e:
                print("An error occurred while parsing net chat messages:", e)
                raise

            conversation_history = "\n".join(
                f"{msg['role'].capitalize()}: {msg['content']}"
                for msg in net_chat_messages
    ...
    def _save_file(self, result: dict | str | Any) -> None:
        ...
        Note:
            ...
            from the magnet_tools package:
                pip install 'magnet[tools]'
                from magnet_tools import FileWriterTool
        ...
        Raises:
            ...
            RuntimeError: If there is an error writing to the file. For cross-platform
                compatibility, especially on Windows, use FileWriterTool from magnet_tools
                package.
        """
        ...
        filewriter_recommendation = (
            ...
            "use FileWriterTool from magnet_tools package."
        )
