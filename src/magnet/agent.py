...
from magnet.agents import CacheHandler
from magnet.agents.agent_builder.base_agent import BaseAgent
from magnet.agents.net_agent_executor import NetAgentExecutor
...
from magnet.knowledge.utils.knowledge_utils import extract_knowledge_context
from magnet.lite_agent import LiteAgent, LiteAgentOutput
from magnet.llm import BaseLLM
from magnet.memory.contextual.contextual_memory import ContextualMemory
from magnet.rag.embeddings.types import EmbedderConfig
from magnet.security import Fingerprint
from magnet.task import Task
from magnet.tools import BaseTool
from magnet.tools.agent_tools.agent_tools import AgentTools
from magnet.utilities import Converter, Prompts
from magnet.utilities.agent_utils import (
    get_tool_names,
    load_agent_from_repository,
    parse_tools,
    render_text_description_and_args,
)
from magnet.utilities.constants import TRAINED_AGENTS_DATA_FILE, TRAINING_DATA_FILE
from magnet.utilities.converter import generate_model_description
from magnet.utilities.llm_utils import create_llm
from magnet.utilities.token_counter_callback import TokenCalcHandler
...
class Agent(BaseAgent):
    ...
    Attributes:
            agent_executor: An instance of the NetAgentExecutor class.
            ...
            function_calling_llm: The language model that will handle the tool calling for this agent, it overrides the net function_calling_llm.
    ...
    net_knowledge_context: str | None = Field(
        default=None,
        description="Knowledge context for the net.",
    )
    ...
    def set_knowledge(self, net_embedder: EmbedderConfig | None = None):
        try:
            if self.embedder is None and net_embedder:
                self.embedder = net_embedder
    ...
    def _is_any_available_memory(self) -> bool:
        ...
        return any(getattr(self.net, attr) for attr in memory_attributes)
    ...
    def execute_task(
        ...
        if self.reasoning:
            try:
                from magnet.utilities.reasoning_handler import (
                    AgentReasoning,
        ...
        if self._is_any_available_memory():
            ...
            contextual_memory = ContextualMemory(
                self.net._short_term_memory,
                self.net._long_term_memory,
                self.net._entity_memory,
                self.net._external_memory,
                agent=self,
                task=task,
            )
        ...
        if self.knowledge or (self.net and self.net.knowledge):
            magnet_event_bus.emit(
                self,
            ...
            try:
                ...
                if self.knowledge_search_query:
                    ...
                    # Quering net specific knowledge
                    knowledge_snippets = self.net.query_knowledge(
                        [self.knowledge_search_query], **knowledge_config
                    )
                    if knowledge_snippets:
                        self.net_knowledge_context = extract_knowledge_context(
                            knowledge_snippets
                        )
                        if self.net_knowledge_context:
                            task_prompt += self.net_knowledge_context
                        ...
                        event=KnowledgeRetrievalCompletedEvent(
                            ...
                            retrieved_knowledge=(
                                ...
                                + (
                                    "\n"
                                    if self.agent_knowledge_context
                                    and self.net_knowledge_context
                                    else ""
                                )
                                + (self.net_knowledge_context or "")
                            ),
                        ),
                    )
            except Exception as e:
        ...
        if self.net and self.net._train:
    ...
    def create_agent_executor(
        ...
        Returns:
            An instance of the NetAgentExecutor class.
        """
        ...
        self.agent_executor = NetAgentExecutor(
            llm=self.llm,
            task=task,
            agent=self,
            net=self.net,
    ...
    def get_multimodal_tools(self) -> Sequence[BaseTool]:
        from magnet.tools.agent_tools.add_image_tool import AddImageTool

        return [AddImageTool()]

    def get_code_execution_tools(self):
        try:
            from magnet_tools import CodeInterpreterTool  # type: ignore
            ...
            self._logger.log(
                "info", "Coding tools not available. Install magnet_tools. "
            )
    ...
    def _training_handler(self, task_prompt: str) -> str:
        ...
        if data := NetTrainingHandler(TRAINING_DATA_FILE).load():
    ...
    def _use_trained_data(self, task_prompt: str) -> str:
        ...
        if data := NetTrainingHandler(TRAINED_AGENTS_DATA_FILE).load():
