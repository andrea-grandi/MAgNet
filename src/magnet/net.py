...
from magnet.agents.cache import CacheHandler
from magnet.nets.net_output import NetOutput
from magnet.events.event_bus import magnet_event_bus
from magnet.events.event_listener import EventListener
from magnet.events.listeners.tracing.trace_listener import (
    TraceCollectionListener,
)
from magnet.events.listeners.tracing.utils import (
    is_tracing_enabled,
...
from magnet.flow.flow_trackable import FlowTrackable
from magnet.knowledge.knowledge import Knowledge
from magnet.knowledge.source.base_knowledge_source import BaseKnowledgeSource
from magnet.llm import LLM, BaseLLM
from magnet.memory.entity.entity_memory import EntityMemory
from magnet.memory.external.external_memory import ExternalMemory
from magnet.memory.long_term.long_term_memory import LongTermMemory
from magnet.memory.short_term.short_term_memory import ShortTermMemory
from magnet.process import Process
from magnet.rag.embeddings.types import EmbedderConfig
from magnet.rag.types import SearchResult
from magnet.security import Fingerprint, SecurityConfig
from magnet.task import Task
from magnet.tasks.conditional_task import ConditionalTask
from magnet.tasks.task_output import TaskOutput
from magnet.tools.agent_tools.agent_tools import AgentTools
from magnet.tools.base_tool import BaseTool, Tool
from magnet.types.usage_metrics import UsageMetrics
from magnet.utilities import I18N, FileHandler, Logger, RPMController
from magnet.utilities.constants import NOT_SPECIFIED, TRAINING_DATA_FILE
from magnet.utilities.net.models import NetContext
from magnet.utilities.evaluators.net_evaluator_handler import NetEvaluator
from magnet.utilities.evaluators.task_evaluator import TaskEvaluator
from magnet.utilities.formatter import (
    aggregate_raw_outputs_from_task_outputs,
    aggregate_raw_outputs_from_tasks,
)
from magnet.utilities.llm_utils import create_llm
from magnet.utilities.planning_handler import NetPlanner
from magnet.utilities.task_output_storage_handler import TaskOutputStorageHandler
from magnet.utilities.training_handler import NetTrainingHandler

warnings.filterwarnings("ignore", category=SyntaxWarning, module="pysbd")


class Net(FlowTrackable, BaseModel):
    """
    Represents a group of agents, defining how they should collaborate and the
    tasks they should perform.

    Attributes:
        tasks: list of tasks assigned to the net.
        agents: list of agents part of this net.
        manager_llm: The language model that will run manager agent.
        manager_agent: Custom agent that will be used as manager.
        memory: Whether the net should use memory to store memories of it's
            execution.
        cache: Whether the net should use a cache to store the results of the
            tools execution.
        function_calling_llm: The language model that will run the tool calling
            for all the agents.
        process: The process flow that the net will follow (e.g., sequential,
            hierarchical).
        verbose: Indicates the verbosity level for logging during execution.
        config: Configuration settings for the net.
        max_rpm: Maximum number of requests per minute for the net execution to
            be respected.
        prompt_file: Path to the prompt json file to be used for the net.
        id: A unique identifier for the net instance.
        ...
        step_callback: Callback to be executed after each step for every agents
            execution.
        share_net: Whether you want to share the complete net information and
            execution with netAI to make the library better, and allow us to
            train models.
        planning: Plan the net execution and add the plan to the net.
        chat_llm: The language model used for orchestrating chat interactions
            with the net.
        security_config: Security configuration for the net, including
            fingerprinting.
    """
    ...
    name: str | None = Field(default="net")
    ...
    memory: bool = Field(
        default=False,
        description="If net should use memory to store memories of it's execution",
    )
    short_term_memory: InstanceOf[ShortTermMemory] | None = Field(
        default=None,
        description="An Instance of the ShortTermMemory to be used by the Net",
    )
    long_term_memory: InstanceOf[LongTermMemory] | None = Field(
        default=None,
        description="An Instance of the LongTermMemory to be used by the Net",
    )
    entity_memory: InstanceOf[EntityMemory] | None = Field(
        default=None,
        description="An Instance of the EntityMemory to be used by the Net",
    )
    external_memory: InstanceOf[ExternalMemory] | None = Field(
        default=None,
        description="An Instance of the ExternalMemory to be used by the Net",
    )
    embedder: EmbedderConfig | None = Field(
        default=None,
        description="Configuration for the embedder to be used for the net.",
    )
    ...
    share_net: bool | None = Field(default=False)
    ...
    before_kickoff_callbacks: list[
        ...
        description=(
            "List of callbacks to be executed before net kickoff. "
            "It may be used to adjust inputs before the net is executed."
        ),
    )
    after_kickoff_callbacks: list[Callable[[NetOutput], NetOutput]] = Field(
        default_factory=list,
        description=(
            "List of callbacks to be executed after net kickoff. "
            "It may be used to adjust the output of the net."
        ),
    )
    max_rpm: int | None = Field(
        default=None,
        description=(
            "Maximum number of requests per minute for the net execution "
            "to be respected."
        ),
    )
    prompt_file: str | None = Field(
        default=None,
        description="Path to the prompt json file to be used for the net.",
    )
    ...
    planning: bool | None = Field(
        default=False,
        description="Plan the net execution and add the plan to the net.",
    )
    ...
    knowledge_sources: list[BaseKnowledgeSource] | None = Field(
        default=None,
        description=(
            "Knowledge sources for the net. Add knowledge sources to the "
            "knowledge object."
        ),
    )
    chat_llm: str | InstanceOf[BaseLLM] | Any | None = Field(
        default=None,
        description="LLM used to handle chatting with the net.",
    )
    knowledge: Knowledge | None = Field(
        default=None,
        description="Knowledge for the net.",
    )
    security_config: SecurityConfig = Field(
        default_factory=SecurityConfig,
        description="Security configuration for the net, including fingerprinting.",
    )
    ...
    tracing: bool | None = Field(
        default=False,
        description="Whether to enable tracing for the net.",
    )
    ...
    def set_private_attrs(self) -> "Net":
        ...
        if (
            ...
            trace_listener.setup_listeners(magnet_event_bus)
    ...
    def _initialize_default_memories(self):
        ...
        self._entity_memory = self.entity_memory or EntityMemory(
            net=self, embedder_config=self.embedder
        )

    @model_validator(mode="after")
    def create_net_memory(self) -> "Net":
        """Initialize private memory attributes."""
        self._external_memory = (
            # External memory does not support a default value since it was
            # designed to be managed entirely externally
            self.external_memory.set_net(self) if self.external_memory else None
        )
    ...
    @model_validator(mode="after")
    def create_net_knowledge(self) -> "Net":
        """Create the knowledge for the net."""
    ...
    def check_config(self):
        """Validates that the net is properly configured with agents and tasks."""
    ...
    def validate_end_with_at_most_one_async_task(self):
        """Validates that the net ends with at most one asynchronous task."""
        ...
        if final_async_task_count > 1:
            raise PydanticCustomError(
                "async_task_count",
                "The net must end with at most one asynchronous task.",
                {},
            )
    ...
    @model_validator(mode="after")
    def validate_must_have_non_conditional_task(self) -> "Net":
        """Ensure that a net has at least one non-conditional task."""
        if not self.tasks:
            return self
        ...
        if non_conditional_count == 0:
            raise PydanticCustomError(
                ...
                "Net must include at least one non-conditional task",
                {},
            )
        return self

    @model_validator(mode="after")
    def validate_first_task(self) -> "Net":
    ...
    @model_validator(mode="after")
    def validate_async_tasks_not_async(self) -> "Net":
    ...
    def fingerprint(self) -> Fingerprint:
        """
        Get the net's fingerprint.

        Returns:
            Fingerprint: The net's fingerprint
        """
    ...
    def _setup_for_training(self, filename: str) -> None:
        ...
        NetTrainingHandler(TRAINING_DATA_FILE).initialize_file()
        NetTrainingHandler(filename).initialize_file()

    def train(
        self, n_iterations: int, filename: str, inputs: dict[str, Any] | None = None
    ) -> None:
        """Trains the net for a given number of iterations."""
        inputs = inputs or {}
        try:
            magnet_event_bus.emit(
                self,
                NetTrainStartedEvent(
                    net_name=self.name,
            ...
            train_net = self.copy()
            train_net._setup_for_training(filename)

            for n_iteration in range(n_iterations):
                train_net._train_iteration = n_iteration
                train_net.kickoff(inputs=inputs)

            training_data = NetTrainingHandler(TRAINING_DATA_FILE).load()

            for agent in train_net.agents:
                if training_data.get(str(agent.id)):
                    result = TaskEvaluator(agent).evaluate_training_data(
                        training_data=training_data, agent_id=str(agent.id)
                    )
                    NetTrainingHandler(filename).save_trained_data(
            ...
            magnet_event_bus.emit(
                self,
                NetTrainFailedEvent(error=str(e), net_name=self.name),
            )
            self._logger.log("error", f"Training failed: {e}", color="red")
            NetTrainingHandler(TRAINING_DATA_FILE).clear()
            NetTrainingHandler(filename).clear()
            raise
    ...
    def kickoff(
        ...
        ctx = baggage.set_baggage(
            "net_context", NetContext(id=str(self.id), key=self.key)
        )
        token = attach(ctx)
        ...
        try:
            ...
            magnet_event_bus.emit(
                self,
                NetKickoffStartedEvent(net_name=self.name, inputs=inputs),
            )

            # Starts the net to work on its assigned tasks.
            ...
            self._set_allow_magnet_trigger_context_for_first_task()

            i18n = I18N(prompt_file=self.prompt_file)

            for agent in self.agents:
                agent.i18n = i18n
                # type: ignore[attr-defined] # Argument 1 to "_interpolate_inputs" of "Net" has incompatible type "dict[str, Any] | None"; expected "dict[str, Any]"
                agent.net = self  # type: ignore[attr-defined]
                agent.set_knowledge(net_embedder=self.embedder)
            ...
            magnet_event_bus.emit(
                self,
                NetKickoffFailedEvent(error=str(e), net_name=self.name),
            )
            raise
        finally:
            detach(token)

    def kickoff_for_each(self, inputs: list[dict[str, Any]]) -> list[NetOutput]:
        """Executes the Net's workflow for each input and aggregates results."""
        results: list[NetOutput] = []

        # Initialize the parent net's usage metrics
        total_usage_metrics = UsageMetrics()

        for input_data in inputs:
            net = self.copy()

            output = net.kickoff(inputs=input_data)

            if net.usage_metrics:
                total_usage_metrics.add_usage_metrics(net.usage_metrics)
    ...
    async def kickoff_async(self, inputs: dict[str, Any] | None = None) -> NetOutput:
        """Asynchronous kickoff method to start the net execution."""
        inputs = inputs or {}
        return await asyncio.to_thread(self.kickoff, inputs)

    async def kickoff_for_each_async(self, inputs: list[dict]) -> list[NetOutput]:
        net_copies = [self.copy() for _ in inputs]

        async def run_net(net, input_data):
            return await net.kickoff_async(inputs=input_data)

        tasks = [
            asyncio.create_task(run_net(net_copies[i], inputs[i]))
    ...
    def _handle_net_planning(self):
        """Handles the Net planning."""
        self._logger.log("info", "Planning the net execution")
        result = NetPlanner(
            tasks=self.tasks, planning_agent_llm=self.planning_llm
        )._handle_net_planning()
    ...
    def _run_sequential_process(self) -> NetOutput:
        """Executes tasks sequentially and returns the final output."""
        return self._execute_tasks(self.tasks)

    def _run_hierarchical_process(self) -> NetOutput:
    ...
    def _execute_tasks(
        ...
        Returns:
            NetOutput: Final output of the net
        """
        ...
        return self._create_net_output(task_outputs)
    ...
    def _create_net_output(self, task_outputs: list[TaskOutput]) -> NetOutput:
        if not task_outputs:
            raise ValueError("No task outputs available to create net output.")

        # Filter out empty outputs and get the last valid one as the main output
        valid_outputs = [t for t in task_outputs if t.raw]
        if not valid_outputs:
            raise ValueError("No valid task outputs available to create net output.")
    ...
    def replay(self, task_id: str, inputs: dict[str, Any] | None = None) -> NetOutput:
        """Replay the net execution from a specific task."""
        stored_outputs = self._task_output_handler.load()
        if not stored_outputs:
            raise ValueError(f"Task with id {task_id} not found in the net's tasks.")
    ...
    def query_knowledge(
        ...
        """Query the net's knowledge base for relevant information."""
        if self.knowledge:
    ...
    def copy(self):
        """
        Creates a deep copy of the Net instance.

        Returns:
            Net: A new instance with copied components
        """
    ...
    def test(
        ...
        """Test and evaluate the Net with the given inputs for n iterations.

        Uses concurrent.futures for concurrent execution.
        """
        try:
            # Create LLM instance and ensure it's of type LLM for NetEvaluator
            ...
            evaluator = NetEvaluator(test_net, llm_instance)

            for i in range(1, n_iterations + 1):
                evaluator.set_iteration(i)
                test_net.kickoff(inputs=inputs)

            evaluator.print_net_evaluation_result()
            ...
            magnet_event_bus.emit(
                self,
                NetTestFailedEvent(error=str(e), net_name=self.name),
            )
            raise

    def __repr__(self):
        return (
            f"Net(id={self.id}, process={self.process}, "
            f"number_of_agents={len(self.agents)}, "
            f"number_of_tasks={len(self.tasks)})"
        )

    def reset_memories(self, command_type: str) -> None:
        """Reset specific or all memories for the net.
        ...
        for config in memory_systems.values():
            if (system := config.get("system")) is not None:
                ...
                try:
                    ...
                    self._logger.log(
                        "info",
                        f"[Net ({self.name if self.name else self.id})] "
    ...
    def _get_memory_systems(self):
        ...
        # Get knowledge for net and agents
        net_knowledge = getattr(self, "knowledge", None)
        net_and_agent_knowledges = (
            [net_knowledge] if net_knowledge is not None else []
        ) + agent_knowledges
        ...
        return {
            ...
            "knowledge": {
                "system": net_and_agent_knowledges
                if net_and_agent_knowledges
                else None,
                "reset": knowledge_reset,
                "name": "Net Knowledge and Agent Knowledge",
            },
            "agent_knowledge": {
    ...
    def reset_knowledge(self, knowledges: list[Knowledge]) -> None:
        """Reset net and agent knowledge storage."""
        for ks in knowledges:
            ks.reset()

    def _set_allow_magnet_trigger_context_for_first_task(self):
        magnet_trigger_payload = self._inputs and self._inputs.get(
            "magnet_trigger_payload"
        )
        able_to_inject = (
            self.tasks and self.tasks[0].allow_magnet_trigger_context is None
        )

        if (
            self.process == Process.sequential
            and magnet_trigger_payload
            and able_to_inject
        ):
            self.tasks[0].allow_magnet_trigger_context = True
