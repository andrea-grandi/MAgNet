...
from magnet import Agent, Net
from magnet.agents.agent_builder.base_agent import BaseAgent
from magnet.experimental.evaluation import AgentEvaluator, create_default_evaluator
from magnet.experimental.evaluation.evaluation_display import (
    AgentAggregatedEvaluationResult,
)
from magnet.experimental.evaluation.experiment.result import (
    ExperimentResult,
    ExperimentResults,
)
from magnet.experimental.evaluation.experiment.result_display import (
...
class ExperimentRunner:
    ...
    def run(
        ...
        if agents is None:
            raise ValueError("Agents must be provided either directly or via a net")
        ...
        for test_case in self.dataset:
            ...
            result = self._run_test_case(test_case=test_case, net=net, agents=agents)
    ...
    def _run_test_case(
        ...
        try:
            self.display.console.print(
                f"[dim]Running net with input: {str(inputs)[:50]}...[/dim]"
            )
            self.display.console.print("\n")
            if net:
                net.kickoff(inputs=inputs)
            else:
                for agent in agents:
