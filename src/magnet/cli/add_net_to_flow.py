...
from magnet.cli.utils import copy_template


def add_crew_to_flow(crew_name: str) -> None:
    """Add a new net to the current flow."""
    ...
    # Create the net within the flow's crews directory
    create_embedded_crew(crew_name, parent_folder=crews_folder)

    click.echo(
        f"Net {crew_name} added to the current flow successfully!",
    )


def create_embedded_crew(crew_name: str, parent_folder: Path) -> None:
    """Create a new net within an existing flow project."""
    ...
    if crew_folder.exists():
        if not click.confirm(
            f"Net {folder_name} already exists. Do you want to override it?"
        ):
            click.secho("Operation cancelled.", fg="yellow")
            return
        click.secho(f"Overriding net {folder_name}...", fg="green", bold=True)
    else:
        click.secho(f"Creating net {folder_name}...", fg="green", bold=True)
        crew_folder.mkdir(parents=True)

    # Create config and net.py files
    config_folder = crew_folder / "config"
    config_folder.mkdir(exist_ok=True)

    templates_dir = Path(__file__).parent / "templates" / "net"
    ...
    src_file = templates_dir / "net.py"
    dst_file = crew_folder / crew_template_file
    copy_template(src_file, dst_file, crew_name, class_name, folder_name)

    click.secho(
        f"Net {crew_name} added to the flow successfully!", fg="green", bold=True
    )
