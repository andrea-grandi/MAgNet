...
from magnet.cli.constants import ENV_VARS
from magnet.net import Net
from magnet.flow import Flow
...
def _get_project_attribute(
    ...
    try:
        ...
        if not any(True for dep in dependencies if "magnet" in dep):
            raise Exception("magnet is not in the dependencies.")
...
def get_crews(crew_path: str = "net.py", require: bool = False) -> list[Net]:
    """Get the net instances from a file."""
    crew_instances = []
    try:
        import importlib.util
                ...
                if crew_path in files and "cli/templates" not in root:
                    ...
                    try:
                        ...
                        try:
                            ...
                            # If we found net instances, break out of the loop
                            if crew_instances:
                                break
                        ...
                        if require:
                            console.print(
                                f"Error importing net from {crew_path}: {e!s}",
                                style="bold red",
                            )
                        continue

            # If we found net instances in this search path, break out of the search paths loop
            if crew_instances:
                break

        if require and not crew_instances:
            console.print("No valid Net instance found in net.py", style="bold red")
            raise SystemExit

    except Exception as e:
        if require:
            console.print(
                f"Unexpected error while loading net: {e!s}", style="bold red"
            )
            raise SystemExit from e
    return crew_instances


def get_crew_instance(module_attr) -> Net | None:
    if (
        callable(module_attr)
        and hasattr(module_attr, "is_crew_class")
        and module_attr.is_crew_class
    ):
        return module_attr().net()
    try:
    ...
    if isinstance(module_attr, Net):
        return module_attr
    return None


def fetch_crews(module_attr) -> list[Net]:
    crew_instances: list[Net] = []
...
def is_valid_tool(obj):
    from magnet.tools.base_tool import Tool
...
def _print_no_tools_warning():
    ...
    console.print(
        ...
        "[green]from magnet.tools import BaseTool, tool\n\n"
        "# Tool class example\n"
