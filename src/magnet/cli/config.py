...
from magnet.cli.constants import (
    MAGNET_ENTERPRISE_DEFAULT_OAUTH2_AUDIENCE,
    MAGNET_ENTERPRISE_DEFAULT_OAUTH2_CLIENT_ID,
    MAGNET_ENTERPRISE_DEFAULT_OAUTH2_DOMAIN,
    MAGNET_ENTERPRISE_DEFAULT_OAUTH2_PROVIDER,
    DEFAULT_MAGNET_ENTERPRISE_URL,
)
from magnet.cli.shared.token_manager import TokenManager

logger = getLogger(__name__)

DEFAULT_CONFIG_PATH = Path.home() / ".config" / "magnet" / "settings.json"
...
def get_writable_config_path() -> Path | None:
    ...
    Tries in order:
    1. Default: ~/.config/magnet/settings.json
    2. Temp directory: /tmp/magnet_settings.json (or OS equivalent)
    3. Current directory: ./magnet_settings.json
    ...
    fallback_paths = [
        ...
        Path(tempfile.gettempdir()) / "magnet_settings.json",  # Temporary directory
        Path.cwd() / "magnet_settings.json",  # Current working directory
    ]

    for config_path in fallback_paths:
        try:
            config_path.parent.mkdir(parents=True, exist_ok=True)
            test_file = config_path.parent / ".magnet_write_test"
            try:
...
DEFAULT_CLI_SETTINGS = {
    "enterprise_base_url": DEFAULT_MAGNET_ENTERPRISE_URL,
    "oauth2_provider": MAGNET_ENTERPRISE_DEFAULT_OAUTH2_PROVIDER,
    "oauth2_audience": MAGNET_ENTERPRISE_DEFAULT_OAUTH2_AUDIENCE,
    "oauth2_client_id": MAGNET_ENTERPRISE_DEFAULT_OAUTH2_CLIENT_ID,
    "oauth2_domain": MAGNET_ENTERPRISE_DEFAULT_OAUTH2_DOMAIN,
}
...
class Settings(BaseModel):
    enterprise_base_url: str | None = Field(
        ...
        description="Base URL of the Magnet Enterprise instance",
    )
