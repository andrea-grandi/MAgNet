...
class ConsoleFormatter:
    current_net_tree: Tree | None = None
    ...
    def __init__(self, verbose: bool = False):
        ...
        # Live instance to dynamically update a Tree renderable (e.g. the Net tree)
    ...
    def update_net_tree(
        ...
        """Handle net tree updates with consistent formatting."""
        if not self.verbose or tree is None:
            return

        if status == "completed":
            prefix, style = "✅ Net:", "green"
            title = "Net Completion"
            content_title = "Net Execution Completed"
        elif status == "failed":
            prefix, style = "❌ Net:", "red"
            title = "Net Failure"
            content_title = "Net Execution Failed"
        else:
            prefix, style = "🚀 Net:", "cyan"
            title = "Net Execution"
            content_title = "Net Execution Started"
    ...
    def create_net_tree(self, net_name: str, source_id: str) -> Tree | None:
        """Create and initialize a new net tree with initial status."""
        if not self.verbose:
            return None

        tree = Tree(
            Text("🚀 Net: ", style="cyan bold") + Text(net_name, style="cyan")
        )
        ...
        self.print_panel(content, "Net Execution Started", "cyan")

        # Set the current_net_tree attribute directly
        self.current_net_tree = tree

        return tree

    def create_task_branch(
        self, net_tree: Tree | None, task_id: str, task_name: str | None = None
    ) -> Tree | None:
        ...
        task_branch = None
        if net_tree:
            task_branch = net_tree.add(task_content)
            self.print(net_tree)
        else:
    ...
    def update_task_status(
        ...
        if not self.verbose or net_tree is None:
            return
        ...
        # Update tree label
        for branch in net_tree.children:
    ...
    def create_agent_branch(
        self, task_branch: Tree | None, agent_role: str, net_tree: Tree | None
    ) -> Tree | None:
        """Create and initialize an agent branch."""
        if not self.verbose or not task_branch or not net_tree:
            return None
    ...
    def get_llm_tree(self, tool_name: str):
        ...
        tree = self.current_flow_tree or self.current_net_tree
    ...
    def handle_tool_usage_started(
        ...
        # Render full net tree when available for consistent live updates
        tree_to_use = self.current_net_tree or net_tree or branch_to_use

        if branch_to_use is None or tree_to_use is None:
            # If we don't have a valid branch, default to net_tree if provided
            if net_tree is not None:
                branch_to_use = tree_to_use = net_tree
            else:
                return None
    ...
    def handle_tool_usage_finished(
        ...
        # Decide which tree to render: prefer full net tree, else parent branch
        tree_to_use = self.current_net_tree or net_tree or self.current_task_branch
    ...
    def handle_llm_call_failed(
        self, tool_branch: Tree | None, error: str, net_tree: Tree | None
    ) -> None:
    ...
    def handle_net_test_started(
        self, net_name: str, source_id: str, n_iterations: int
    ) -> Tree | None:
        """Handle net test started event."""
        if not self.verbose:
            return None

        # Create initial panel
        content = Text()
        content.append("🧪 Starting Net Test\n\n", style="blue bold")
        content.append("Net: ", style="white")
        content.append(f"{net_name}\n", style="blue")
        ...
        test_label.append(net_name or "Net", style="blue")
        test_label.append("\nStatus: ", style="white")
    ...
    def handle_net_test_completed(
        self, flow_tree: Tree | None, net_name: str
    ) -> None:
        """Handle net test completed event."""
        if not self.verbose:
            return
        ...
        if flow_tree:
            ...
            test_label.append(net_name or "Net", style="green")
            test_label.append("\nStatus: ", style="white")
        ...
        completion_content.append("Net: ", style="white")
        completion_content.append(f"{net_name}\n", style="green")
        completion_content.append("\nStatus: ", style="white")
        completion_content.append("Completed", style="green")

        self.print_panel(completion_content, "Test Completion", "green")

    def handle_net_train_started(self, net_name: str, timestamp: str) -> None:
        """Handle net train started event."""
        if not self.verbose:
            return

        content = Text()
        content.append("📋 Net Training Started\n", style="blue bold")
        content.append("Net: ", style="white")
        content.append(f"{net_name}\n", style="blue")
    ...
    def handle_net_train_completed(self, net_name: str, timestamp: str) -> None:
        """Handle net train completed event."""
        if not self.verbose:
            return

        content = Text()
        content.append("✅ Net Training Completed\n", style="green bold")
        content.append("Net: ", style="white")
        content.append(f"{net_name}\n", style="green")
        content.append("Time: ", style="white")
    ...
    def handle_net_train_failed(self, net_name: str) -> None:
        """Handle net train failed event."""
        if not self.verbose:
            return

        failure_content = Text()
        failure_content.append("❌ Net Training Failed\n", style="red bold")
        failure_content.append("Net: ", style="white")
        failure_content.append(net_name or "Net", style="red")

        self.print_panel(failure_content, "Training Failure", "red")
        self.print()

    def handle_net_test_failed(self, net_name: str) -> None:
        """Handle net test failed event."""
        if not self.verbose:
            return

        failure_content = Text()
        failure_content.append("❌ Net Test Failed\n", style="red bold")
        failure_content.append("Net: ", style="white")
        failure_content.append(net_name or "Net", style="red")
    ...
    def handle_knowledge_retrieval_started(
        ...
        tree_to_use = branch_to_use or net_tree
    ...
    def handle_knowledge_query_failed(
        ...
        tree_to_use = self.current_lite_agent_branch or net_tree
        branch_to_use = self.current_lite_agent_branch or agent_branch
    ...
    def handle_reasoning_started(
        ...
        # We always want to render the full net tree when possible so the
        # Live view updates coherently. Fallbacks: net tree → branch itself.
        tree_to_use = self.current_net_tree or net_tree or branch_to_use
    ...
    def handle_agent_logs_execution(
        ...
        from magnet.agents.parser import AgentAction, AgentFinish
    ...
    def handle_memory_save_started(
        self, agent_branch: Tree | None, net_tree: Tree | None
    ) -> None:
        if not self.verbose:
            return
