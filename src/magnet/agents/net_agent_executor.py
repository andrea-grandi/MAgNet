...
from magnet.agents.agent_builder.base_agent_executor_mixin import NetAgentExecutorMixin
...
from magnet.utilities import I18N, Printer
...
from magnet.utilities.constants import TRAINING_DATA_FILE
from magnet.utilities.tool_utils import execute_tool_and_check_finality
from magnet.utilities.training_handler import NetTrainingHandler


class NetAgentExecutor(NetAgentExecutorMixin):
    """Executor for network agents.
    ...
    def _show_start_logs(self) -> None:
        ...
        magnet_event_bus.emit(
            ...
            AgentLogsStartedEvent(
                ...
                or (hasattr(self, "net") and getattr(self.net, "verbose", False)),
            ),
        )
    ...
    def _handle_net_training_output(
        ...
        train_iteration = (
            getattr(self.net, "_train_iteration", None) if self.net else None
        )
        ...
        training_handler = NetTrainingHandler(TRAINING_DATA_FILE)
    ...
    def _is_training_mode(self) -> bool:
        ...
        """
        return bool(self.net and self.net._train)
    ...
    def _handle_training_feedback(
        ...
        Returns:
            Improved answer.
        """
        self._handle_net_training_output(initial_answer, feedback)
        ...
        self._handle_net_training_output(improved_answer)
        self.ask_for_human_input = False
