...
from magnet import Net
from magnet.project.utils import memoize

"""Decorators for defining net components and their behaviors."""


def before_kickoff(func):
    """Marks a method to execute before net kickoff."""
    func.is_before_kickoff = True
    return func


def after_kickoff(func):
    """Marks a method to execute after net kickoff."""
    func.is_after_kickoff = True
    return func


def task(func):
    """Marks a method as a net task."""
    func.is_task = True
...
def tool(func):
    """Marks a method as a net tool."""
    func.is_tool = True
    return memoize(func)


def callback(func):
    """Marks a method as a net callback."""
...
def net(func) -> Callable[..., Net]:
    """Marks a method as the main net execution point."""

    @wraps(func)
    def wrapper(self, *args, **kwargs) -> Net:
        instantiated_tasks = []
        instantiated_agents = []
        agent_roles = set()
        ...
        for _, callback in self._before_kickoff.items():
            net.before_kickoff_callbacks.append(callback_wrapper(callback, self))
        for _, callback in self._after_kickoff.items():
            net.after_kickoff_callbacks.append(callback_wrapper(callback, self))
