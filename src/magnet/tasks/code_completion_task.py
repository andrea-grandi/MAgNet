from magnet.utils.utils import (
    parse_code_completion, 
    check_function_result, 
    most_similar_code,
    is_final_answer_correct
)
from magnet.nets.nets import Nets
from human_eval.data import read_problems


class CodeCompletion(Nets):
    ...
    def get_final_answer(self, idxs, question_data):
        ...
        if len(passed_codes) != 0:
            ...
            _, most_similar_answer, max_score = most_similar_code(passed_answers)
        ...
        _, most_similar_answer, max_score = most_similar_code(pred_answers)
        print("Most Similar Answer: {}\nMax bleu score: {}".format(most_similar_answer, max_score))
        return most_similar_answer

    def evaluation(self, df):
        return df.apply(is_final_answer_correct, axis=1).mean()