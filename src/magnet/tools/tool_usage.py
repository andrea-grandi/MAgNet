...
from magnet.telemetry import Telemetry
from magnet.tools.structured_tool import NetStructuredTool
from magnet.tools.tool_calling import InstructorToolCalling, ToolCalling
from magnet.utilities import I18N, Converter, Printer
...
class ToolUsage:
    ...
    def use(
        ...
        if (
            isinstance(tool, NetStructuredTool)
    ...
    def _use(
        ...
        if self.agent:
            ...
            magnet_event_bus.emit(self, ToolUsageStartedEvent(**event_data))
    ...
    def on_tool_error(
        ...
        magnet_event_bus.emit(self, ToolUsageErrorEvent(**{**event_data, "error": e}))
    ...
    def on_tool_use_finished(
        ...
        if self.task:
            event_data["task_id"] = str(self.task.id)
            event_data["task_name"] = self.task.name or self.task.description
        magnet_event_bus.emit(self, ToolUsageFinishedEvent(**event_data))
