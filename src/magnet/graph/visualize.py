...
def GPTSwarmVis(graph, style="pyvis", dry_run: bool = False, file_name=None):
    ...
    if style == "pyvis":
        ...
        net = Network(notebook=True, height="750px", width="100%", bgcolor="#FFFFFF", font_color="black", directed=True) # type: ignore
        for node_id, node in graph.nodes.items():
            if hasattr(graph, 'graphs'):
                graph_id = ''
                ...
                color_key = node_id
            net.add_node(node_id, label=f"{type(node).__name__}\n(ID: {node_id})", color=color_map[color_key]) # type: ignore
        ...
        if not dry_run:
            import os
            from magnet.utils.const import GPTSWARM_ROOT
        ...
        node_colors = [color_map[node] for node in G.nodes()] # type: ignore
        node_sizes = [3000 + 100 * G.degree[node] for node in G.nodes()] # type: ignore
