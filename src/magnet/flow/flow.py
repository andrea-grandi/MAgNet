...
from magnet.flow.flow_visualizer import plot_flow
from magnet.flow.persistence.base import FlowPersistence
from magnet.flow.types import FlowExecutionData
from magnet.flow.utils import get_possible_return_constants
...
class Flow(Generic[T], metaclass=FlowMeta):
    ...
    async def _execute_start_method(self, start_method_name: str) -> None:
        ...
        - Automatically injects magnet_trigger_payload if available in flow inputs
        """
    ...
    def _inject_trigger_payload_for_start_method(
        ...
        def prepare_kwargs(*args, **kwargs):
            ...
            trigger_payload = inputs.get("magnet_trigger_payload")

            sig = inspect.signature(original_method)
            accepts_trigger_payload = "magnet_trigger_payload" in sig.parameters

            if trigger_payload is not None and accepts_trigger_payload:
                kwargs["magnet_trigger_payload"] = trigger_payload
            elif trigger_payload is not None:
                self._log_flow_event(
                    f"Trigger payload available but {original_method.__name__} doesn't accept magnet_trigger_payload parameter",
                    color="yellow",
                )
            return args, kwargs
    ...
    def plot(self, filename: str = "magnet_flow") -> None:
        magnet_event_bus.emit(
            self,
            FlowPlotEvent(
                type="flow_plot",
