...
def method_calls_net(method: Any) -> bool:
    """
    Check if the method contains a call to `.net()`.

    Parameters
    ----------
    method : Any
        The method to analyze for net() calls.

    Returns
    -------
    bool
        True if the method calls .net(), False otherwise.

    Notes
    -----
    Uses AST analysis to detect method calls, specifically looking for
    attribute access of 'net'.
    """
    try:
    ...
    class NetCallVisitor(ast.NodeVisitor):
        """AST visitor to detect .net() method calls."""
        ...
        def visit_Call(self, node):
            if isinstance(node.func, ast.Attribute):
                if node.func.attr == "net":
                    self.found = True
            self.generic_visit(node)

    visitor = NetCallVisitor()
    visitor.visit(tree)
...
def add_nodes_to_network(
    ...
    for method_name, (x, y) in node_positions.items():
        ...
        elif method_calls_net(method):
            node_style = node_styles["net"]
        else:
